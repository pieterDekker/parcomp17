...
static void error(char *errmsg) { /* print error message an abort program */
    fprintf(stderr, errmsg);
    exit(EXIT_FAILURE);
}

static void *safeMalloc(int n) { /* wrapper function for malloc with error checking */
    void *ptr = malloc(n);
    if (ptr == NULL) {
...
static Image makeImage(int w, int h) { /* routine for constructing (memory allocation) of images */
    Image im;
    int row;
...
static void freeImage(Image im) { /* routine for deallocating memory occupied by an image */
    int row;
...
static Image readPGM(char *filename) { /* routine that returns an image that is read from a PGM file */
...
static void writePGM(Image im, char *filename) { /* routine that writes an image to a PGM file.
...
static void threshold(int th, int less, int greater, Image image) { /* routine for converting a greyscale image into a binary image
...
static void distanceBlur(int background, Image image) { /* blur the image by assigning to each pixel a gray value
...
static double
PearsonCorrelation(Image image, Image mask) { /* returns the Pearson correlation coefficient of image and mask. */
...
static int isEmptyRow(int row, int col0, int col1,
                      int background, Image image) { /* returns TRUE if and only if all pixels im[row][c]
...
static int isEmptyColumn(int row0, int row1, int col,
                         int background, Image image) { /* returns TRUE if and only if all pixels im[r[col]
...
static void makeCharImage(int row0, int col0, int row1, int col1,
                          ...
                          Image mask) { /* construct a (centered) mask image from a segmented character */
...
static int findCharacter(int background, Image image,
                         int row0, int row1, int *col0, int *col1) { /* find the bounding box of the left most character
...
static void characterSegmentation(int background,
                                  int row0, int row1, Image image) { /* Segment a linestrip into characters and perform
...
static int findLineStrip(int background, int *row0, int *row1,
                         Image image) { /* find the first line strip that is encountered
...
static void lineSegmentation(int background, Image page) { /* Segments a page into line strips. For each line strip
...
static void constructAlphabetMasks() { /* Construct a full set of alphabet symbols from the file
